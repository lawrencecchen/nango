...
const databaseUrl = envs.ORCHESTRATOR_DATABASE_URL ||
...
try {
    ...
    const eventsHandler = new EventsHandler({
        CREATED: (task) => logger.info(`Task created: ${stringifyTask(task)}`),
        STARTED: (task) => logger.info(`Task started: ${stringifyTask(task)}`),
        SUCCEEDED: (task) => logger.info(`Task succeeded: ${stringifyTask(task)}`),
        FAILED: (task) => logger.error(`Task failed: ${stringifyTask(task)}`),
        EXPIRED: (task) => logger.error(`Task expired: ${stringifyTask(task)}`),
        CANCELLED: (task) => logger.info(`Task cancelled: ${stringifyTask(task)}`)
    });

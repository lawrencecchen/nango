...
class DryRunService {
    constructor(environment, returnOutput = false) {
        if (environment) {
    ...
    run(options, debug = false) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { success, error, response: config } = yield configService.load(process.cwd(), debug);
            if (!success || !config) {
            ...
            if (!providerConfigKey) {
                providerConfigKey = (_a = config.find((config) => [...config.syncs, ...config.actions].find((sync) => sync.name === syncName))) === null || _a === void 0 ? void 0 : _a.providerConfigKey;
                if (!providerConfigKey) {
                    providerConfigKey =
                        ((_b = config.find((config) => {
                        ...
                        })) === null || _b === void 0 ? void 0 : _b.providerConfigKey) || '';
                    isPostConnectionScript = true;
                }
                if (!providerConfigKey) {
                    console.log(chalk.red(`Provider config key not found, please check that the provider exists for this sync name: ${syncName} by going to the Nango dashboard.`));
                    return;
                }
            }
            ...
            const nangoConnection = (yield getConnection(providerConfigKey, connectionId, {
                'Nango-Is-Sync': true,
            ...
            const { config: { provider } } = yield getConfig(providerConfigKey, debug);
            if (!provider) {
            ...
            let type = 'sync';
            if ((syncInfo === null || syncInfo === void 0 ? void 0 : syncInfo.type) === SyncConfigType.ACTION) {
                type = 'action';
            }
            if (isPostConnectionScript) {
                type = 'post-connection-script';
            }
            const result = yield compileAllFiles({ fullPath: process.cwd(), debug, scriptName: syncName, providerConfigKey, type });
            if (!result) {
            ...
            const syncRun = new syncRunService({
                ...
                isAction: (syncInfo === null || syncInfo === void 0 ? void 0 : syncInfo.type) === SyncConfigType.ACTION,
            ...
            try {
                ...
                const results = yield syncRun.run(lastSyncDate, true, secretKey, process.env['NANGO_HOSTPORT']);
                let resultOutput = '';
                if (results) {
                ...
                if (syncRun.logMessages && syncRun.logMessages.messages.length > 0) {
                    ...
                    while (index < syncRun.logMessages.messages.length) {
                        ...
                        const confirmation = yield promptly.confirm(`There are ${remaining} logs messages remaining. Would you like to see the next 10 log messages? (y/n)`);
                        if (confirmation) {
                            displayBatch();
                        }
                        else {

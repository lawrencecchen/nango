...
export function compileAllFiles({ debug, fullPath, scriptName, providerConfigKey, type }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { success: loadSuccess, error, response: config } = yield configService.load(fullPath, debug);
        ...
        for (const file of integrationFiles) {
            try {
                const completed = yield compile({ fullPath, file, config, modelNames, compiler, debug });
                if (!completed) {
...
export function compileSingleFile({ fullPath, file, config, modelNames, tsconfig, debug = false }) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const compiler = tsNode.create({
                skipProject: true,
...
function compileImportedFile({ fullPath, filePath, compiler, type, modelNames }) {
...
function compile({ fullPath, file, config, modelNames, compiler, debug = false }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const type = (syncConfig === null || syncConfig === void 0 ? void 0 : syncConfig.type) || SyncConfigType.SYNC;
        ...
        yield build({
            ...
            onSuccess: () => __awaiter(this, void 0, void 0, function* () {
                if (fs.existsSync(file.outputPath)) {
                    yield fs.promises.rename(file.outputPath, outputPath);
...
export function listFilesToCompile({ fullPath, scriptDirectory, scriptName, config, debug }) {
    let files = [];
    if (scriptName) {
        if (debug) {

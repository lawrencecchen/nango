...
describe('InMemoryKVStore', () => {
    ...
    it('should not allow overriding a key', async () => {
        await store.set('key', 'value');
        await expect(store.set('key', 'value2', false)).rejects.toEqual(new Error('Key already exists'));
    });

    it('should return null for a key that has expired', async () => {
        const ttlInMs = 1000;
        await store.set('key', 'value', true, ttlInMs);

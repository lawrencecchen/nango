...
export var UserType;
(function (UserType) {
    UserType["Local"] = "localhost";
    UserType["SelfHosted"] = "self-hosted";
    UserType["Cloud"] = "cloud";
...
export function dirname(thisFile) {
    return path.dirname(fileURLToPath(thisFile || import.meta.url));
}
export function parseTokenExpirationDate(expirationDate) {
...
export function isTokenExpired(expireDate, bufferInSeconds) {
...
export function getOauthCallbackUrl(environmentId) {
    return __awaiter(this, void 0, void 0, function* () {
        const globalCallbackUrl = getGlobalOAuthCallbackUrl();
        if (environmentId != null) {
            const environment = yield environmentService.getById(environmentId);
            return (environment === null || environment === void 0 ? void 0 : environment.callback_url) || globalCallbackUrl;
        }
        return globalCallbackUrl;
    });
}
export function getAppCallbackUrl(_environmentId) {
    return __awaiter(this, void 0, void 0, function* () {
 ...
 */
export function interpolateString(str, replacers) {
    return str.replace(/\${([^{}]*)}/g, (a, b) => {
        const r = replacers[b];
        return typeof r === 'string' || typeof r === 'number' ? r : a; // Typecast needed to make TypeScript happy
    });
}
export function interpolateStringFromObject(str, replacers) {
    return str.replace(/\${([^{}]*)}/g, (a, b) => {
        const r = b.split('.').reduce((o, i) => o[i], replacers);
        return typeof r === 'string' || typeof r === 'number' ? r : a;
    });
}
export function connectionCopyWithParsedConnectionConfig(connection) {
    const connectionCopy = Object.assign({}, connection);
    const rawConfig = connectionCopy.connection_config;
...
export function mapProxyBaseUrlInterpolationFormat(baseUrl) {
...
export function interpolateIfNeeded(str, replacers) {
    if (str.includes('${')) {
...
export function getConnectionConfig(queryParams) {
...
export function safeStringify(obj) {
    ...
    const content = obj.map((arg) => (typeof arg === 'object' ? stringify(arg) : String(arg))).join(' ');
    return content;
}

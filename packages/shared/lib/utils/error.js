...
export class NangoError extends Error {
    constructor(type, payload = {}, status) {
        super();
        this.status = 500;
        this.type = type;
        ...
        switch (type) {
            ...
            case 'error_loading_nango_config':
                this.status = 400;
                this.message = `Error loading nango config. Please make sure it is present and formatted correctly.${this.payload ? ` Error: ${JSON.stringify(this.payload, null, 2)}` : ''}`;
                break;
...
export const formatScriptError = (err, errorType, scriptName) => {
    var _a;
    let errorMessage;

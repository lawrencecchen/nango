...
export function getActionOrModelByEndpoint(nangoConnection, method, path) {
    return __awaiter(this, void 0, void 0, function* () {
        const config = yield configService.getProviderConfig(nangoConnection.provider_config_key, nangoConnection.environment_id);
        if (!config) {
        ...
        const result = yield schema()
            ...
            [`${SYNC_CONFIG_TABLE}.nango_config_id`]: config.id,
...
export function getOpenApiSpec(environment_id) {
    return __awaiter(this, void 0, void 0, function* () {
        const rows = yield schema()
        ...
        rows.forEach((row) => {
            ...
            if (type === SyncConfigType.SYNC) {
                queryParams.push({ name: 'delta', in: 'query', schema: { type: 'string', format: 'date-time' } }, { name: 'offset', in: 'query', schema: { type: 'integer' } }, { name: 'limit', in: 'query', schema: { type: 'integer' } }, { name: 'sort_by', in: 'query', schema: { type: 'string' } }, { name: 'order', in: 'query', schema: { type: 'string' } }, { name: 'filter', in: 'query', schema: { type: 'string' } });
            }
            const schemaProps = (_a = modelSchema
                .find((model) => model.name === modelName)) === null || _a === void 0 ? void 0 : _a['fields'].reduce((acc, field) => {

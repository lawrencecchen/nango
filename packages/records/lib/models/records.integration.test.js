...
describe('Records service', () => {
    ...
    afterEach(() => __awaiter(void 0, void 0, void 0, function* () {
        yield db(RECORDS_TABLE).truncate();
    }));
    it('Should write records', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const res = yield upsertRecords(records, connectionId, model, syncId, 1);
        expect(res).toStrictEqual({ addedKeys: ['1', '2', '3', '4'], updatedKeys: [], deletedKeys: [], nonUniqueKeys: ['1'] });
        const newRecords = [{ id: '2', name: 'Jane Moe' }];
        const updateRes = yield updateRecords(newRecords, connectionId, model, syncId, 2);
        expect(updateRes).toStrictEqual({ addedKeys: [], updatedKeys: ['2'], deletedKeys: [], nonUniqueKeys: [] });
    }));
    it('Should be able to encrypt and insert 2000 records under 2 seconds', () => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        const connectionId = 1;
        const model = 'my-model';
        const syncId = '00000000-0000-0000-0000-000000000000';
        ...
        expect((_a = res.deletedKeys) === null || _a === void 0 ? void 0 : _a.length).toStrictEqual(0);
    ...
    it('Should delete records', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        yield upsertRecords(records, connectionId, model, syncId, 1);
        const toDelete = [
            { id: '1', name: 'John Doe' },
            { id: '2', name: 'Jane Doe' }
        ];
        const res1 = yield upsertRecords(toDelete, connectionId, model, syncId, 1, true);
        expect(res1).toStrictEqual({ addedKeys: [], updatedKeys: [], deletedKeys: ['1', '2'], nonUniqueKeys: [] });
        // Try to delete the same records again
        // Should not have any effect
        const res2 = yield upsertRecords(toDelete, connectionId, model, syncId, 1, true);
        expect(res2).toStrictEqual({ addedKeys: [], updatedKeys: [], deletedKeys: [], nonUniqueKeys: [] });
    }));
    it('Should retrieve records', () => __awaiter(void 0, void 0, void 0, function* () {
        var _b;
        const n = 10;
        const { connectionId, model } = yield upsertNRecords(n);
        const response = yield Records.getRecords({ connectionId, model });
        if (response.isErr()) {
            throw new Error('Response is undefined');
        }
        const { records, next_cursor } = response.value;
        expect(records.length).toBe(n);
        expect((_b = records[0]) === null || _b === void 0 ? void 0 : _b['_nango_metadata']).toMatchObject({
    ...
    it('Should paginate the records to retrieve all records', () => __awaiter(void 0, void 0, void 0, function* () {
        var _c, _d;
        const numOfRecords = 3000;
        const limit = 100;
        const { connectionId, model } = yield upsertNRecords(numOfRecords);
        let cursor = null;
        const allFetchedRecords = [];
        do {
            const response = yield Records.getRecords(Object.assign({ connectionId,
                model,
                limit }, (cursor && { cursor })));
            ...
            expect(records === null || records === void 0 ? void 0 : records.length).toBeLessThanOrEqual(limit);
        } while (cursor);
        for (let i = 1; i < allFetchedRecords.length; i++) {
            const currentRecordDate = dayjs((_c = allFetchedRecords[i]) === null || _c === void 0 ? void 0 : _c._nango_metadata.first_seen_at);
            const previousRecordDate = dayjs((_d = allFetchedRecords[i - 1]) === null || _d === void 0 ? void 0 : _d._nango_metadata.first_seen_at);
    ...
    it('Should be able to retrieve 20K records in under 5s with a cursor', () => __awaiter(void 0, void 0, void 0, function* () {
...
function upsertNRecords(n) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield upsertRecords(records, connectionId, model, '00000000-0000-0000-0000-000000000000', 1);
        return {
            connectionId,
            model,
            syncId,
...
function upsertRecords(records, connectionId, model, syncId, syncJobId, softDelete = false) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const upsertRes = yield Records.upsert({ records: formatRes.value, connectionId, model, softDelete });
        if (upsertRes.isErr()) {
...
function updateRecords(records, connectionId, model, syncId, syncJobId) {
    return __awaiter(this, void 0, void 0, function* () {
        const formatRes = formatRecords({ data: records, connectionId, model, syncId, syncJobId });
        if (formatRes.isErr()) {
            throw new Error(`Failed to format records: ${formatRes.error.message}`);
        }
        const updateRes = yield Records.update({ records: formatRes.value, connectionId, model });
        if (updateRes.isErr()) {
...
//# sourceMappingURL=records.integration.test.js.map
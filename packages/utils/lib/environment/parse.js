...
export const ENVS = z.object({
    ...
    // API
    NANGO_PORT: z.coerce.number().optional().default(3003),
    ...
    NANGO_ENCRYPTION_KEY: z
        .string({
        required_error: 'To learn more about NANGO_ENCRYPTION_KEY, please read the doc at https://docs.nango.dev/host/self-host/self-hosting-instructions#encrypt-sensitive-data'
    })
        .optional(),
...
export function parseEnvs(schema, envs = process.env) {
...
function zodErrorToString(issues) {
    return issues
        .map((issue) => {

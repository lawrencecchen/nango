...
export default function fetchData(nango) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            for (var _c = __asyncValues(nango.paginate({
                ...
                for (const workspace of workspaces) {
                    ...
                    if (workspace.name !== 'Personal Projects') {
                        ...
                        try {
                            for (var _e = (e_2 = void 0, __asyncValues(nango.paginate({ endpoint: '/api/1.0/users', params, retries: 10 }))), _f; _f = yield _e.next(), !_f.done;) {
                                const users = _f.value;
                                const normalizedUsers = users.map((user) => toUser(user));
                                yield nango.batchSave(normalizedUsers, 'User');
                            }
                        }

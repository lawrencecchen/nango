        ...
        try {
            ...
            try {
                for (var _c = __asyncValues(nango.paginate(Object.assign(Object.assign({}, config), { endpoint }))), _d; _d = yield _c.next(), !_d.done;) {
                    const application = _d.value;
                    const mappedApplication = application.map(mapApplication) || [];
                    const batchSize = mappedApplication.length;
                    totalRecords += batchSize;
                    yield nango.log(`Saving batch of ${batchSize} application(s) (total application(s): ${totalRecords})`);
                    yield nango.batchSave(mappedApplication, 'GreenhouseApplication');
                }
            }

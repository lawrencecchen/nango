            ...
            try {
                for (var _b = __asyncValues(nango.paginate(Object.assign(Object.assign({}, config), { endpoint }))), _c; _c = yield _b.next(), !_c.done;) {
                    ...
                    for (const item of team) {
                        ...
                        const mappedTeam = mapTeam(item);
                        totalRecords++;
                        teamsToSave.push(mappedTeam);
                    }
                    if (teamsToSave.length > 0) {
                        yield nango.batchSave(teamsToSave, 'HackerRankWorkTeam');
                        yield nango.log(`Saving batch of ${teamsToSave.length} team(s) (total team(s): ${totalRecords})`);
                    }
                }
            }
